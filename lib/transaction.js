...
import { split, toArray, includesStringList, capitalize, equalsIgnoreCase } from './utils'
...
export const getActivities = (
  ...
  if (includesStringList(toArray(messages).map(m => m['@type']), ['MsgSend', 'MsgTransfer', 'RetryIBCTransferRequest', 'RouteIBCTransfersRequest', 'MsgUpdateClient', 'MsgAcknowledgement'])) {
    output = toArray(
    ...
    output = toArray(
      toArray(messages).flatMap(m => {
        ...
        tx_id = _.head(events)?.tx_id || tx_id
				status = _.head(events)?.status || status
        events =
          toArray(events).flatMap(e =>
            Object.entries(e)
              .filter(([k, v]) => v && typeof v === 'object')
              .map(([k, v]) => { return { event: k, ...v } })
          )
...
export const getSender = (
  ...
  return _.head(
    toArray(
	    toArray([equalsIgnoreCase(_.head(types), 'MsgDelegate') && 'delegator_address', 'sender', 'signer']).map(f =>
