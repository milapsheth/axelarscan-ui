...
export const createDayJSFromUnixtime = time => dayjs(time ? !isNaN(time) ? Number(time) * 1000 : time : undefined)
...
export const normalizeQuote = (
  ...
  split(split(string, 'normal', '"').join(''), 'normal', `'`).join('')
...
export const chartColor = (
  ...
  ['day'].includes(granularity) ? theme === 'dark' ? '#fc9b67' : '#ff7d20' : theme === 'dark' ? '#009ef7' : '#009ef7'
