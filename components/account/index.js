      ...
      const getData = async () => {
        if (address) {
          if (['axelarvaloper', 'axelarvalcons'].findIndex(p => address.startsWith(p)) > -1 && validators_data) {
            const {
              operator_address,
            } = { ...validators_data.find(v => includesStringList(address.toLowerCase(), [v.operator_address, v.consensus_address])) }
    ...
    () => {
      const getData = async () => {
        if (address?.startsWith('axelar1') && address.length < 65 && validators_data) {
          const {
            data,
          } = { ...await getRedelegations({ address }) }
      ...
      const getData = async () => {
        if (address?.startsWith('axelar1') && address.length < 65 && assets_data) {
          const {
            rewards,
            total,
          } = { ...await distributionRewards(address) }
    ...
    () => {
      const getData = async () => {
        if (address?.startsWith('axelar1') && address.length < 65 && assets_data && validators_data) {
          const {
            operator_address,
          } = { ...validators_data.find(v => equalsIgnoreCase(v.delegator_address, address)) }

          if (operator_address) {
            const {
              commission,
            } = { ...await distributionCommissions(operator_address) }
