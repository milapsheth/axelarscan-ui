              ...
              <div className="overflow-x-auto flex flex-wrap items-center">
                {Object.entries(types).map(([k, v]) => (
                  <div
                    ...
                    className={`min-w-max ${toArray(typesFiltered).includes(k) ? 'text-blue-500 dark:text-slate-200 font-semibold' : 'text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 font-normal'} cursor-pointer whitespace-nowrap flex items-center text-xs sm:text-sm space-x-1 mr-3`}
                  >
                    <span>{k}</span>
                    <NumberDisplay
                      value={v}
                      ...
                      value ?
                        <div className="flex flex-wrap items-start">
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            <div className="flex items-center space-x-1">
                              <AccountProfile address={value} url={explorer && `${explorer.url}${explorer.address_path?.replace('{address}', value)}`} />
                          ...
                          <div className="flex items-start space-x-1">
                            ...
                            {sourceTxHash && (
                              <div className="flex items-center space-x-1">
                                ...
                                <ExplorerLink value={sourceTxHash} explorer={source_chain_data.explorer} />
                              </div>
                            )}
                            {contractAddress && (
                              <>
                                <BsArrowRightShort size={18} />
                                <div className="flex items-center space-x-1">
                                  <AccountProfile address={contractAddress} url={explorer && `${explorer.url}${explorer.address_path?.replace('{address}', contractAddress)}`} />
                                ...
                                <div className="max-w-xl flex flex-wrap">
                                  {split(newOwners || newOperators, 'normal', ';').map((a, i) => (
                                    <div key={i} className="flex items-center space-x-1 mb-1 mr-2">
                                      <AccountProfile address={a} url={explorer && `${explorer.url}${explorer.address_path?.replace('{address}', a)}`} />
          ...
          <div className="space-y-2">
            ...
            {executeResponse && (
              <Alert
                ...
                color={_status === 'success' ? 'green' : _status === 'error' ? 'red' : 'blue'}
                icon={_status === 'success' ? <IoCheckmarkCircleOutline size={26} /> : _status === 'error' ? <IoCloseCircleOutline size={26} /> : <IoTimeOutline size={26} />}
                animate={{ mount: { y: 0 }, unmount: { y: 32 } }}
                dismissible={{ onClose: () => setExecuteResponse(null) }}
                className="alert-box flex"
              >
                <div className="flex flex-col text-base ml-1">
                  <span>{message}</span>
                  {hash && (
                    <ExplorerLink
                      value={hash}
                      ...
                      height={18}
                      iconOnly={false}
                      viewOnClassName="font-medium pr-0.5"
                    />
                  )}
                </div>
              </Alert>
