...
export default (
  ...
  const Point = ({ title, name, noTooltip = false }) => {
    const point = (
      <div>
        ...
        <span className="uppercase text-blue-400 dark:text-blue-500 text-sm font-medium">
          {title}
        </span>
      </div>
    )
  ...
  return (
    <div className="bg-white dark:bg-slate-800 bg-opacity-50 dark:bg-opacity-50 rounded flex flex-col sm:flex-row sm:justify-between space-y-2 sm:space-y-0 sm:space-x-2 p-4">
      <div className="w-44 space-y-0.5">
        ...
        <NumberDisplay
          ...
          format="0,0.00a"
          suffix=" transactions"
          className="whitespace-nowrap text-slate-500 dark:text-slate-300 text-sm"
        />
      </div>
      {total > 0 && (
                    ...
                    <Tooltip placement="bottom" content={label || name}>
                      <div>
                        <TimeSpent
                          ...
                          toTime={time_spent}
                          noTooltip={true}
                          className="whitespace-nowrap text-black dark:text-white text-xs font-medium"
                        />
                      </div>
                    </Tooltip> :
                    <div />
