      ...
      return (
        <div className="w-fit space-y-3">
          {activities.map((a, i) => {
            ...
            const valueClassName = 'flex items-center text-slate-600 dark:text-slate-200 text-sm font-medium space-x-1'
            const titleClassName = 'text-black dark:text-white font-semibold'
                          ...
                          {Object.entries({ ...e }).filter(([k, v]) => !['event'].includes(k)).map(([k, v], i) => (
                            <div key={i} className="flex items-start space-x-4">
                              ...
                              <div className="flex items-start space-x-1">
                                <div className="max-w-xl break-all text-black dark:text-white font-medium">
                                  {typeof v === 'string' ?
                                    ellipse(v, 256) :
                                    v && typeof v === 'object' ?
                      ...
                      <div key={j} className="min-w-max bg-slate-100 dark:bg-slate-800 rounded space-y-2 py-5 px-4">
                        ...
                        {attributes.filter(([k, v]) => typeof v !== 'undefined').map(([k, v], i) => (
                          <div key={i} className="flex items-start space-x-4">
                            ...
                            <div className="flex items-start space-x-1">
                              <div className="max-w-xl break-all text-black dark:text-white font-medium">
                                {(Array.isArray(v) || (typeof v === 'string' && v.startsWith('[') && v.endsWith(']'))) && ['gateway_address', 'deposit_address', 'token_address', 'tx_id'].includes(k) ?
                                  toHex(JSON.parse(v)) :
                                  typeof v === 'string' ?
                                    ellipse(v, 256) :
                                    v && typeof v === 'object' ?
